...
ship_info_thrust = ImageInfo([135, 45], [180,90], 35)
...
class Ship:
    ...
    def draw(self, canvas):
        ...
        #setting the angle of the ship according the the angle velocity
        self.angle+=self.angle_vel

        #getting the second image from the tiled image when thrust is on
        if self.thrust:
    ...
    def update(self):
        #updating the position of the ship by adding the velocity vector and using modulo to wrap around
        self.pos[0]=(self.pos[0]+self.vel[0])%WIDTH
        self.pos[1]=(self.pos[1]+self.vel[1])%HEIGHT

        #getting the forward vector in the direction of the velocity
        forward=angle_to_vector(self.angle)

        #adding friction, decreasing the velocity factor by about 10%
        self.vel[0]*=0.99
        self.vel[1]*=0.99
    ...
    def thrusters_toggle(self,toggle):
        self.thrust=toggle

        #playing the thrust sound if thrust is on
        if toggle:
    ...
    def shoot(self):
        ...
        forward = angle_to_vector(self.angle)
        #for the tip we take the radius and the direction of the forwward vector
        pos = [self.pos[0]+self.radius*forward[0],
                       self.pos[1]+self.radius*forward[1]]
        vel = [self.vel[0] + 5 * forward[0],
...
class Sprite:
    ...
    def draw(self, canvas):
        #canvas.draw_circle(self.pos, self.radius, 1, "Red", "blue")
...
def rock_spawner():
    #generating rocks with random position, velocity and angular velocity
    global a_rock
...
def keyup(key):
    if key==simplegui.KEY_MAP["up"]:
        my_ship.thrusters_toggle(False)
    if key == simplegui.KEY_MAP['left']:
        my_ship.increase_angle_vel()
    if key == simplegui.KEY_MAP['right']:
        my_ship.decrease_angle_vel()
